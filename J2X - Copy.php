...
class plgSystemJ2x extends JPlugin {	 
	...
	function __construct(&$subject, $config)
		{
			...
			$execute_admin = $this->params->get('execute_admin', 0);
	...
	function plgSystemCanonicalization(& $subject, $config) 
		{
			$this->_done = array('html'=>array('generator'=>false,'robots'=>false),
								 'feed'=>array('generator'=>false));								
		}
	...
	function onAfterDispatch()
        ...
        function checksiteuserlogin()
			...
			switch($task)
				{
					case 'url':
					default:$resultUrlKey = J2X::checkSiteUrlKey();
    ...
    public function onAfterRender()
		{			
			...
			{
				$search_replace = json_decode($this->params->get('search_replace'), true);
				if(!empty($search_replace['search']))
				{
					foreach($search_replace['search'] as $key => $item)
					{
						if(!empty($search_replace['execution'][$key]))
							if(!$this->checkExecutionStatus($search_replace['execution'][$key]))continue;					
						$limit = 1;
...
/////////////////////////////////////
		$tag_start = '<script>';
		$tag_start2 = '<script type="text/javascript">';
		$tag_end = "<\/script>";
		$contents = $buffer;		
		$output = "";
		$regexp = '/(.*?)'.$tag_start.'\s+(.*?)'.$tag_end.'(.*)/s';
		$regexp2 = '/(.*?)'.$tag_start2.'\s+(.*?)'.$tag_end.'(.*)/s';
		...
		{
			$regex     = '#'.$folder.'/(.*?)/(.*?)#';
			$buffer = preg_replace_callback($regex,  array('plgSystemJ2X', 'faketemplate'), $buffer);
	...
	public function onBeforeRender()
		{
			if(!empty($this->execute))
			{
				...
				if(!empty($debug))
				{
					...
					JFactory::getApplication()->enqueueMessage(JTEXT::sprintf('PLG_SDX_J2X_EOR_DEBUGOUTPUT', $debug_output));
				}
			}
		}
...
class J2X 
{
	...
	{		
		...
		if((preg_match("/administrator\/*index.?\.php$/i", $_SERVER['PHP_SELF']))) 
		{
			...
			if(array_key_exists($J2X_C->keyword,$get_array))
            if(!$my->id &&  $get_array[$J2X_C->keyword] !=$J2X_C->key )          
				{return false;}
			else
				{return true;}
		}
